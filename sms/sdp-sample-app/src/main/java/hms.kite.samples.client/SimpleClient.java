...
//            mtSmsReq.setChargingAmount("5");
            ...
            LOGGER.info("Sms Mt send back to user : " + mtSmsReq);
    ...
    private MtSmsReq createSimpleMtSms(MoSmsReq moSmsReq) {
        ...
        String response = getBmiCalculationResponse(moSmsReq.getMessage());
    ...
    private String getBmiCalculationResponse(String moMessage) {
        /*bmi 172 72.1*/
        final String[] messageParts = moMessage.split(" ");
        final Double height = Double.valueOf(messageParts[1])/100;
        final Double weight = Double.valueOf(messageParts[2]);


        final double bmi = weight / Math.pow(height, 2);

        return "You bmi is " + bmi;
    }
